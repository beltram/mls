...
use base64::Engine;
use bat::PagingMode;
use openmls::prelude::{MlsMessageIn, TlsDeserializeTrait, Welcome};
...
fn decode(draft: u8, msg: Vec<u8>) -> String {
    match draft {
        12 => {
            if let Ok(msg) = MlsMessageIn::try_from_bytes(&msg[..]) {
                return format!("{msg:#?}");
            } else if let Ok(welcome) = Welcome::tls_deserialize(&mut msg.as_slice()) {
